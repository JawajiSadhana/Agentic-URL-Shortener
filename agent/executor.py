...
class Executor:
    def __init__(self):
        self.tools = {"file_write": self.file_write, "code_edit": self.code_edit, "code_search": self.code_search, "shell": self.shell, "test_runner": self.test_runner}
        self.memory = Memory()
        self.retry_cap = 2
        ...
        for attempt in range(1, self.retry_cap+2):
            try:
                ...
                if attempt > self.retry_cap: 
                    return f"Tool failed after {self.retry_cap} retries: {e}" # yaha raise nahi karna
                time.sleep(2**attempt)

    def file_write(self, path, content, evidence=None): os.makedirs(os.path.dirname(path), exist_ok=True); open(path,"w",encoding="utf-8").write(content); log("tool_file_write",{"path":path}, evidence=evidence); return f"Written {path}"
    def code_edit(self, path, find, replace): 
        if not os.path.exists(path): return f"File not found: {path}"
        code=open(path,"r",encoding="utf-8").read().replace(find,replace); open(path,"w",encoding="utf-8").write(code); log("tool_code_edit",{"path":path}); return f"Edited {path}"
